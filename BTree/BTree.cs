...
namespace BTree
{
    public partial class BTreeForm : Form
    {
        ...
        private void Exercise2__Click(object sender, EventArgs e)
        {
            ...
            int[] nums = {12, 6, 18, 37, 31, 44 };

            root = new BTree(25, null);
            root.isData = false;

            foreach (int num in nums)
            {
                node = new BTree(num, root);
                node.isData = false;
            }
        ...
        private void Exercise3__Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < 15; ++i)
            {
                node = new BTree(Convert.ToChar(random.Next(65, 90)), root);
        ...
        private void Exercise4__Click(object sender, EventArgs e)
        {
            ...
            String[] letters = { "F", "C", "B", "A", "E", "D", "J", "I", "H", "G", "L", "K", "P", "O", "N", "T", "S", "R", "Q", "W", "V", "U", "X", "Y", "Z" };

            foreach (String letter in letters)
            {
                node = new BTree(letter, root);
                node.isData = false;
            }
            ...
            for (int i = 0; i < 15; ++i)
            {
                node = new BTree(letters[random.Next(letters.Length)], root);
            }
        ...
        private void Exercise5__Click(object sender, EventArgs e)
        {
            ...
            {
                node = new BTree(letters[i], root);
                node.isData = false;
            }
            node = new BTree("C", null);
            BTree.DeleteNode(node, root);
            this.richTextBox.Text += "\n";
            BTree.TraverseAscending(root);

            node = new BTree("I", null);
            BTree.DeleteNode(node, root);
            this.richTextBox.Text += "\n";
            BTree.TraverseAscending(root);

            node = new BTree("A", null);
            BTree.DeleteNode(node, root);
            this.richTextBox.Text += "\n";
            BTree.TraverseAscending(root);
        ...
        private void Exercise6__Click(object sender, EventArgs e)
        {
            ...
            // Your code here
            node = new BTree(new Person("sarah", 20), null);
            root = node;

            node = new BTree(new Person("Claire", 16), root);
            node = new BTree(new Person("Amy", 41), root);
            node = new BTree(new Person("Luna", 3), root);
            node = new BTree(new Person("Todd", 48), root);
            node = new BTree(new Person("Kai", 20), root);
            node = new BTree(new Person("Kayla", 27), root);
            node = new BTree(new Person("Leo", 14), root);
            node = new BTree(new Person("Rachel", 22), root);
            node = new BTree(new Person("Josh", 22), root);
            node = new BTree(new Person("Paula", 70), root);
            node = new BTree(new Person("Katherine", 90), root);
            node = new BTree(new Person("Lilly", 15), root);
            node = new BTree(new Person("Holly", 45), root);
            node = new BTree(new Person("Heather", 48), root);
            node = new BTree(new Person("Owen", 18), root);
            node = new BTree(new Person("George", 2), root);
