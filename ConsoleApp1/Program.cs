...
// Class: Program
// Author: David Schuh & Sarah Schneider
...
static class Program {
    ...
    start:
        ...
        for (nCntr = 1; nCntr <= nQuestions; ++nCntr) {
            ...
            double start = DateTime.Now.Second;
            switch (nOp) {
                case 0:
                    nAnswer = val1 + val2;
                    sQuestions = $"Question #{nCntr}: {val1 + " (this first value is really hard)"} + {val2} ==> {val1 + val2} ";
                    sQuestions = "Question #" + (nCntr) + ": " + val1 + " + " + val2 + " ==> " + (val1 + val2) + " ";
                    break;
                case 1:
                    nAnswer = val1 - val2;
